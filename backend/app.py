...
# File: backend/app.py (Additions/Modifications)

from flask import Flask, request, jsonify
from flask_cors import CORS
import logging
import os # Import os for file handling if needed later
...
# --- Helper Function for API Responses --- (Keep existing one)
def create_response(data=None, error=None, status_code=200):
    # ... (same as before) ...
    if error:
...
# Keep /api/health
# Keep /api/suggest-test-cases (for text input)
# Keep /api/analyze-defect
# Keep /api/recommend-automation
# Keep /api/analyze-code-change-impact


# --- NEW Endpoint for File Upload ---
@app.route('/api/suggest-test-cases-from-file', methods=['POST'])
def suggest_test_cases_from_file():
    """
    API endpoint to analyze an uploaded file (PDF/DOCX) and suggest test cases.
    ...
    if file and allowed_file(file.filename):
        # filename = secure_filename(file.filename) # Good practice, though not strictly needed if not saving
        app.logger.info(f"Processing uploaded file: {file.filename}")
        file_stream = file.stream # Get the file stream directly
        ...
        try:
            if file_ext == 'pdf':
                ...
                # This case should not be reached due to allowed_file check, but good to have
                return create_response(error="Unsupported file type", status_code=400)

            if not extracted_text.strip():
                 return create_response(error="Could not extract text from the file or file is empty", status_code=400)

            app.logger.info(f"Extracted {len(extracted_text)} characters from {file.filename}")

            # --- Prompt Engineering (Similar to text input, but using extracted content) ---
            # Limit text size to avoid exceeding API limits (adjust limit as needed)
            max_chars = 15000 # Example limit, depends on model and complexity
            if len(extracted_text) > max_chars:
                app.logger.warning(f"Extracted text truncated from {len(extracted_text)} to {max_chars} chars.")
                extracted_text = extracted_text[:max_chars]


            prompt = f"""
            Act as an expert Software Quality Assurance Engineer.
            Analyze the following content extracted from a document ({file.filename}) and generate a list of relevant test case titles or brief descriptions.
            Focus on identifying requirements, features, or user actions described in the text.
            Cover positive scenarios, negative scenarios, boundary values, and potential edge cases based *only* on the provided text content.
            Format the output clearly, preferably as a numbered or bulleted list.
            ...
            if suggestion.startswith("Error:"):
                # Error already logged in gemini_client
                return create_response(error=suggestion, status_code=500)

            app.logger.info(f"Successfully generated suggestions for file {file.filename}")
            return create_response({"suggestions": suggestion, "filename": file.filename})

        except ValueError as ve: # Catch parsing errors
             app.logger.error(f"File parsing error for {file.filename}: {ve}")
             return create_response(error=str(ve), status_code=400)
        except Exception as e:
             app.logger.error(f"Unexpected error processing file {file.filename}: {e}", exc_info=True)
             return create_response(error="An unexpected server error occurred while processing the file.", status_code=500)

    else:
        return create_response(error="File type not allowed. Please upload PDF or DOCX.", status_code=400)


# --- Main Execution Guard --- (Keep existing one)
if __name__ == '__main__':
    # ... (same as before) ...
    app.run(host='0.0.0.0', port=5001, debug=True)